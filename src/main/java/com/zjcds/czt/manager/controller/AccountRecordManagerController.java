...
@Api(description = "核算记录管理类操作")
public class AccountRecordManagerController {
    ...
    public ResponseResult<PageResult<AccountRecordForm.Owner>> querySubsidyPolicies(Paging paging,
        ...
        if (CollectionUtils.isEmpty(orderBys)) {
            ...
            orderBys.add("accountTimeDesc");
        }
        return new ResponseResult<>(accountRecordService.findAccountRecords(paging, queryString, orderBys));
    }

    @GetMapping("/{id}")
    @ApiOperation(value = "根据id查询单个评测记录", produces = "application/json;charset=utf-8")
    public ResponseResult<AccountRecordForm.OwnerWithDetails> querySubsidyPolicy(@PathVariable("id") Integer id) {
        return new ResponseResult<>(accountRecordService.findAccountRecord(id));
    }
